...
class SAF41VectorEngine:
    def __init__(self, n_stages=715):
        self.n = n_stages
        self.feigenbaum_constants = np.array([0.987, 0.933, 0.937, 0.954])
        self.phase_shift = np.pi / 2.0

    def generate_hexagonal_qfs_mesh(self, t):
...
if __name__ == "__main__":
    engine = SAF41VectorEngine()
    x, y, z = engine.generate_hexagonal_qfs_mesh(t=0.0)

    fig = plt.figure(figsize=(10, 8))
    ax = fig.add_subplot(111, projection="3d")
    surf = ax.plot_surface(
        x, y, z, cmap="plasma", edgecolor="k", linewidth=0.1, alpha=0.8
    )

    ax.set_title("SAF V41.0 Hexagonal QFS Grid (N=715)")
