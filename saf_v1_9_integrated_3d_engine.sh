#!/bin/bash
# SAF V1.9 Integrierte 3D-Spektral & Lepton Engine (CRF 19)
set -e
...
# 2. Exakte Audioquelle per Python FS-Walker ermitteln
...
# 3. 3D-Spektral-Grafik als dynamische Textur einbinden (sucht automatisch nach V1.6 / 3D-Ansicht)
BASE_IMAGE=$(python3 -c '
import os
desktop = os.path.expanduser("~/Desktop")
for root, dirs, files in os.walk(desktop):
    for f in files:
        if ("V 1.6" in f or "3D ANSICHT" in f or "saf_3d" in f) and f.endswith(".png"):
            print(os.path.join(root, f))
            break
    else:
        continue
...
echo "=== 3D-Spektral-Grafik geladen: $BASE_IMAGE ==="

OUTPUT_STREAM="$SAF_DIR/compressed/saf_v1_9_integrated_3d_matrix.mp4"
mkdir -p "$(dirname "$OUTPUT_STREAM")"

# 4. FFmpeg Multi-Layer Rendering (Grafik-Loop + Audio-Spektren + CRF 19)
echo "=== SAF V1.9 Integrierte 3D-Pipeline gestartet (CRF 19) ==="
ffmpeg -y -i "$TARGET_AUDIO" -loop 1 -i "$BASE_IMAGE" \
  -filter_complex "
    ...
    [1:v]scale=1920:1080:force_original_aspect_ratio=decrease,
         ...
         zoompan=z='1.001+0.0005*sin(2*PI*time/2.0)':x='(iw-iw/zoom)/2':y='(ih-ih/zoom)/2':d=1:s=1920x1080:fps=60,
         lenscorrection=cx=0.5:cy=0.5:k1=-0.15:k2=0.05,
         hue=h='10*sin(2*PI*t/6)':s=1.4,
         gblur=sigma=0.2:steps=1[bg_3d_matrix];
    ...
    [bg_3d_matrix][wif_frame]overlay=30:30:shortest=1[v_stage1];
...
echo "=== SAF V1.9 Integriertes Rendering erfolgreich! Output: $OUTPUT_STREAM ==="
