#!/bin/bash
# SAF 4D Goldskull Matrix Engine - Fixed Perspective & Dynamic Spectrum
...
OUTPUT_STREAM="$SAF_DIR/saf_4d_goldskull_matrix.mp4"
...
echo "=== SAF 4D Goldskull Matrix Pipeline gestartet ==="
...
ffmpeg -y -i "$AUDIO_INPUT" -f lavfi -i "mandelbrot=s=1920x1080:maxiter=1000:start_x=-0.743643887037:start_y=0.131825904205" \
  -filter_complex "
    ...
    [1:v]format=yuv420p,
         ...
         hue=h='140+30*sin(t*0.5)':s=2.0,
         perspective=x0=W*0.06:y0=H*0.08:x1=W*0.94:y1=H*0.03:x2=W*0.04:y2=H*0.96:x3=W*0.96:y3=H*0.91:interpolation=1[bg_4d_matrix];
    ...
    [bg_4d_matrix][slit1]overlay=(W-w)/2-300:(H-h)/2:shortest=1[v1];
...
echo "=== 4D Goldskull Rendering abgeschlossen! Output: $OUTPUT_STREAM ==="
