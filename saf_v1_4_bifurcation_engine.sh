#!/bin/bash
# SAF V1.4 Spektral-Bifurkations Engine
# Feature: Anti-Aliasing/Edge Smoothing + 5D Continuity & Multi-Level Diffraction Lattice
...
# Fallback: Synthesisiere SAF Spektral-Signal falls keine WAV vorhanden ist
if [ -z "$AUDIO_INPUT" ] || [ ! -f "$AUDIO_INPUT" ]; then
    echo "Keine .wav Datei gefunden. Synthesisiere SAF V1.4 Spektral-Signal..."
...
OUTPUT_STREAM="$SAF_DIR/saf_v1_4_bifurcation_matrix.mp4"

echo "=== SAF V1.4 Multi-Dimensional Integration Pipeline gestartet ==="
...
ffmpeg -y -i "$AUDIO_INPUT" -f lavfi -i "mandelbrot=s=1920x1080:maxiter=1800:start_x=-0.743643887037:start_y=0.131825904205" \
  -filter_complex "
    ...
    [spec_gui]drawbox=x=0:y=0:w=iw:h=ih:color=0x00ffff@0.6:t=2,gblur=sigma=0.8:steps=1[spec_frame_smoothed];
    ...
    [bg_smoothed][spec_frame_smoothed]overlay=40:40:shortest=1[v_spec];
...
echo "=== SAF V1.4 Rendering mit Eckenglättung abgeschlossen! Output: $OUTPUT_STREAM ==="
